...
func main() {
	...
	workerGraphName := os.Getenv("WORKER_GRAPH_NAME")
	if len(workerGraphName) == 0 {
		slog.Error("environment WORKER_GRAPH_NAME invalid, use default name va_openai_azure_http")
		workerGraphName = "va_openai_azure_http"
		//	os.Exit(1)
	}

	workers := internal.NewWorkers(workerGraphName, logPath, log2Stdout, workerQuitTimeoutSeconds)
	workers.Start()
	...
	httpServer := internal.NewHttpServer(httpServerConfig, workers)
	httpServer.Start()
}
